...
def create(parallel=False):
    with open("data/tmp/run_config.json") as json_file:
        create_ds(json.load(json_file), parallel=parallel)       
...
def main():
    ...
    parser.add_argument("-c", "--create", action="store_true", help="creates Zarr, NetCDF4 and HDF5 Files using a previously saved run format")
