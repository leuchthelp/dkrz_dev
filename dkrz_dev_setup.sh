...
spack install --fresh lmod %gcc@11.2.0
. $(spack location -i lmod)/lmod/lmod/init/bash
. $SPACK_ROOT/share/spack/setup-env.sh

# additionals for my own comfort
spack install --fresh git %gcc@11.2.0
module load git
spack install --fresh nano %gcc@11.2.0 

spack install --fresh python@3.11.9 %gcc@11.2.0
module load python
spack install --fresh openmpi %gcc@11.2.0
module load openmpi
spack install --fresh hdf5@1.14.5~cxx~fortran+hl~ipo~java~map+mpi+shared+subfiling~szip+threadsafe+tools %gcc@11.2.0
module load hdf5
spack install --fresh argobots@main %gcc@11.2.0
module load argobots
spack install hdf5-vol-async@develop %gcc@11.2.0 
module load hdf5-vol-async
spack install netcdf-c build_system=cmake %gcc@11.2.0
module load netcdf-c
spack install py-pip %gcc@11.2.0
module load py-pip
spack install py-netcdf4 %gcc@11.2.0
module load py-netcdf4
spack install py-h5py %gcc@11.2.0
module load py-h5py
spack install py-mpi4py %gcc@11.2.0
module load py-mpi4py
spack install py-numpy %gcc@11.2.0
module load py-numpy
spack install py-rich %gcc@11.2.0
module load py-rich
