...
void create_hdf5_async(bool with_chunking, int size)
{
     ...
     */
    file_id = H5Fcreate_async("data/datasets/test_dataset_hdf5-c.h5", H5F_ACC_TRUNC, H5P_DEFAULT, plist_id, es_id);
    ...
    status = H5Dwrite_async(dset_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wbuf, es_id);
...
void create_hdf5_parallel(int argc, char **argv, bool with_chunking, int size)
{
    ...
    // setup dimensions
    int process_mem_size = size / mpi_size;

    dims[0] = size;
    mem_chunk_dims[0] = process_mem_size;
    filespace = H5Screate_simple(1, dims, NULL);
...
void create_hdf5_async_parallel(int argc, char **argv, bool with_chunking, int size)
{
    hid_t plist_id = 0;
    hid_t file_id = 0;
    hid_t filespace = 0;
     ...
     * Create a new file collectively.
     */
    file_id = H5Fcreate_async("data/datasets/test_dataset_hdf5-c_async.h5", H5F_ACC_TRUNC, H5P_DEFAULT, plist_id, es_id);
...
void bench_variable_hdf5_parallel(int argc, char **argv, int size, int iteration)
{
    ...
    for (int i = 0; i < iteration; i++)
    {
         ...
         * H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_INDEPENDENT);
         */
        status = H5Dread(dset_id, H5T_NATIVE_FLOAT, H5S_BLOCK, H5S_ALL, plist_id, rbuf);
...
void bench_variable_async(int size, int iteration)
{
    ...
    for (int i = 0; i < iteration; i++)
    {
         ...
         * Open existing file collectively and release property list identifier.
         */
        file_id = H5Fopen_async("data/datasets/test_dataset_hdf5-c.h5", H5F_ACC_RDONLY, plist_id, es_id);
        H5Pclose(plist_id);
         ...
         * H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_INDEPENDENT);
         */
        status = H5Dread_async(dset_id, H5T_NATIVE_FLOAT, H5S_BLOCK, H5S_ALL, H5P_DEFAULT, rbuf, es_id);
...
void bench_variable_async_parallel(int argc, char **argv, int size, int iteration)
{
    double arr3[iteration];

    /*
    printf(ANSI_COLOR_YELLOW "Create hdf5 file with vol-async" ANSI_COLOR_RESET "\n");
    create_hdf5_async_parallel(argc, argv, false, size);
    printf(ANSI_COLOR_YELLOW "Finish creating hdf5 file" ANSI_COLOR_RESET "\n");
    */
    ...
    for (int i = 0; i < iteration; i++)
    {
         ...
         * H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_INDEPENDENT);
         */
        status = H5Dread_async(dset_id, H5T_NATIVE_FLOAT, H5S_BLOCK, H5S_ALL, plist_id, rbuf, es_id);
    ...
    save_to_json(arr3, "test_hdf5-c_async_parallel.json", "hdf5-c-async-read-parallel", iteration);
}
...
int main(int argc, char **argv)
{
    ...
    case 5:
        ...
        break;
    case 6:
        printf(ANSI_COLOR_BLUE "Running hdf5 parallel benchmark with a filesize of %d for %d iterations" ANSI_COLOR_RESET "\n", size, iterations);
        bench_variable_hdf5_parallel(argc, argv, size, iterations);
        MPI_Finalize();
        break;
    case 7:
        printf(ANSI_COLOR_RED "PLEASE ENSURE YOU LOADED HDF5-VOL-ASYNC AND SET THE ENVIRONMENTAL VARIABLES REQUIRED CORRECTLY" ANSI_COLOR_RESET "\n");
        printf(ANSI_COLOR_BLUE "Running hdf5-vol-async parallel benchmark with a filesize of %d for %d iterations" ANSI_COLOR_RESET "\n", size, iterations);
        bench_variable_async_parallel(argc, argv, size, iterations);
        MPI_Finalize();
        break;
    case ARGP_KEY_ARG:
        return 0;
